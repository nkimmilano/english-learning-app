    ...
    <div className="flex flex-col gap-4 w-full">
      ...
      <div className="grid grid-cols-1 gap-2">
        {shuffled.map((opt, i) => {
          ...
          return (
            <motion.button key={opt} whileTap={{ scale: 0.98 }} onClick={() => choose(opt)}
              className="rounded-2xl p-3 text-base font-semibold transition-all flex items-start gap-3 text-left" style={s}>
              <span className="w-7 h-7 rounded-lg flex items-center justify-center text-sm font-bold font-orbitron shrink-0 mt-0.5"
                style={{ background: 'rgba(0,200,255,0.1)', color: '#00c8ff' }}>{labels[i]}</span>
      ...
      <AnimatePresence>
        {revealed && (
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col gap-3">
            ...
            <motion.button whileTap={{ scale: 0.97 }} onClick={() => onDone(correct)}
...
export default function ListeningStory({ drill, onAnswer }: Props) {
  ...
  async function playStory() {
    ...
    if (hasElevenLabs) {
      setIsLoading(true);
      const url = await fetchElevenLabsAudio(drill.passage ?? '');
      setIsLoading(false);
      if (url) {
    ...
    return (
      <div className="flex flex-col items-center gap-6 w-full max-w-xl">
        ...
        <motion.button
          ...
          style={isLoading ? {
            background: 'rgba(0,200,255,0.06)', border: '3px solid rgba(0,200,255,0.3)', color: '#6b6b9a',
          } : isPlaying ? {
            background: 'rgba(255,0,128,0.15)', border: '3px solid #ff0080', color: '#ff0080',
        ...
        {isPlaying && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex gap-1 items-end h-6">
            {[1,2,3,4,5,6,7].map(i => (
              <motion.div key={i} className="w-1.5 rounded-full" style={{ background: '#00c8ff' }}
        ...
        {hasListened && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col gap-3 w-full">
            <motion.button whileTap={{ scale: 0.97 }} onClick={() => setPhase('questions')}
              className="py-4 rounded-2xl font-black text-xl w-full cyber-btn">
              Answer Questions 📝
            </motion.button>
            <button onClick={() => setShowScript(s => !s)} className="text-sm font-medium underline"
              style={{ color: '#6b6b9a' }}>
              {showScript ? '▲ Hide script' : '▼ Show script'}
            </button>
            {showScript && (
              <div className="rounded-3xl p-5 w-full" style={{ background: '#12121a', border: '1px solid rgba(0,245,255,0.18)' }}>
                <p className="text-sm leading-relaxed" style={{ color: '#b0b0d0' }}>{drill.passage}</p>
              </div>
            )}
          </motion.div>
        )}
  ...
  if (phase === 'done') {
    ...
    return (
      <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }}
        ...
        style={{ background: '#12121a', border: `2px solid ${score >= 75 ? '#00ff88' : '#ff6b00'}`,
          boxShadow: `0 0 40px ${score >= 75 ? 'rgba(0,255,136,0.2)' : 'rgba(255,107,0,0.2)'}` }}>
        <span className="text-7xl">{score === 100 ? '🌟' : score >= 75 ? '🎧' : '💪'}</span>
        <h2 className="text-2xl font-black font-orbitron" style={{ color: '#00f5ff' }}>Listening Complete!</h2>
        <p className="text-5xl font-black font-orbitron" style={{ color: score >= 75 ? '#00ff88' : '#ff6b00' }}>{score}%</p>
  ...
  return (
    <div className="flex flex-col gap-4 w-full max-w-xl">
      <button onClick={isPlaying ? stopStory : playStory} disabled={isLoading}
        ...
        {isLoading ? '⟳ Loading...' : isPlaying ? '⏹ Stop' : '🔄 Replay Story'}
      </button>
      <AnimatePresence mode="wait">
        <motion.div key={qIndex} initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }}
          exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.2 }}>
          <QuestionPanel q={questions[qIndex]} index={qIndex} total={questions.length} onDone={handleSubAnswer} />
        </motion.div>
      </AnimatePresence>
    </div>
  );
